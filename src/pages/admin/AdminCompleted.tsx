...
export default function AdminCompleted() {
  ...
  const moveToCancelled = async () => {
    if (selectedIds.size === 0) return;
    setMoving(true);
    for (const id of selectedIds) {
      await supabase.rpc('update_order_status', { p_order_id: id, p_status: 'cancelled' });
    }
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="font-serif text-3xl text-ink-900">Completed Orders</h1>
          <p className="text-sm text-ink-500 mt-1">Orders that have completed the HHC process</p>
        </div>
        {selectedIds.size > 0 && (
          <button onClick={moveToCancelled} disabled={moving} className="btn-primary inline-flex items-center gap-2 bg-red-600 hover:bg-red-700">
            {moving ? <Loader2 size={14} className="animate-spin" /> : <ArrowRight size={14} />}
            Move to Cancelled ({selectedIds.size})
          </button>
        )}
      </div>
      ...
      {loading ? (
        <div className="text-center text-ink-400 text-sm py-20">Loading completed orders...</div>
      ) : orders.length === 0 ? (
        <div className="text-center text-ink-400 text-sm py-20">No completed orders found.</div>
      ) : (
        <div className="overflow-x-auto bg-white border border-ink-100">
          <table className="w-full text-sm">
            ...
            <tbody>
              {orders.map((order) => (
                <tr key={order.id} className="border-b border-ink-50 hover:bg-ivory-50/50">
                  <td className="p-4">
          ...
          <div className="bg-ivory-50 w-full max-w-2xl my-8 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-ink-100 sticky top-0 bg-ivory-50 z-10">
              <div>
                ...
                <p className="text-2xs text-ink-500 mt-1">Status: Completed | HHC ID: {selectedOrder.hhc_id ?? '—'}</p>
              </div>
